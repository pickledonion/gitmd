...
<input id="sidebar-search-input" type="search" autocomplete="off" autocapitalize="off" spellcheck="false" aria-label="Filter sidebar" data-on:keydown="if (evt.key !== 'Escape') evt.stopPropagation()" data-on:input="const query = evt.currentTarget.value.toLowerCase(); document.querySelectorAll('.file-browser li,.history li,.outline li').forEach(item => item.hidden = !item.textContent.toLowerCase().includes(query))">
</form>
</aside>
