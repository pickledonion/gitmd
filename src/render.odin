...
render_outline :: proc(commit: ^Commit) -> string {
	builder := strings.builder_make()
	defer strings.builder_destroy(&builder)
	strings.write_string(&builder, `<section id="outline" class="outline" aria-label="Document outline" data-show="$sidebar === 'outline'"><ol>`)
	position := 0
...
render_history :: proc(history: ^History, selected_commit: int) -> string {
	builder := strings.builder_make()
	defer strings.builder_destroy(&builder)
	strings.write_string(&builder, `<section id="history" class="history" aria-label="Commit history" data-show="$sidebar === 'history'"><ol>`)
	for commit, index in history.commits {
...
<li><a id="commit-%s"%s href="%s?commit=%s&amp;sidebar=history" data-on:click="evt.preventDefault(); clearTimeout($historyTimer); const current = evt.currentTarget.closest('.history').querySelector('a.selected'); if (current !== evt.currentTarget) {{ current?.classList.remove('selected'); current?.removeAttribute('aria-current'); evt.currentTarget.classList.add('selected'); evt.currentTarget.setAttribute('aria-current', 'page') }}; $selected = %d; @get('/snapshot/%s'); history.replaceState(null, '', evt.currentTarget.href)">
<span class="commit-subject">%s</span><span class="commit-meta"><code>%s</code><time datetime="%s">%s</time></span>
</a></li>`, commit.full_hash, selected_class, repository_url(history.path), commit.short_hash, index, commit.full_hash, html_escape(commit.subject), commit.short_hash, html_escape(commit.author_date), date)
	}
...
.file-browser,.history,.outline { flex:1; min-height:0; overflow:auto; overflow-anchor:none; scroll-padding-block:1px; }
.file-browser ol { list-style:none; padding:8px; margin:0; }.file-browser a { display:flex; gap:8px; align-items:center; padding:8px; border-radius:6px; color:inherit; text-decoration:none; overflow-wrap:anywhere; }.file-browser a:focus-visible { background:color-mix(in srgb,var(--selected) 55%,transparent); outline:none; }.file-browser a.selected { background:var(--selected); }.file-icon { color:var(--muted); font-family:ui-monospace,SFMono-Regular,Consolas,monospace; }
...
.history a:focus-visible { background:color-mix(in srgb,var(--selected) 55%,transparent); outline:none; }.history a.selected { background:var(--selected); }
