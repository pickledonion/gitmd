...
render_outline :: proc(commit: ^Commit) -> string {
	builder := strings.builder_make()
	defer strings.builder_destroy(&builder)
	strings.write_string(&builder, `<section id="outline" class="outline" aria-label="Document outline" data-preserve-attr="style" data-show="$sidebar === 'outline'">`)
...
render_history :: proc(history: ^History, selected_commit: int) -> string {
	builder := strings.builder_make()
	defer strings.builder_destroy(&builder)
	strings.write_string(&builder, `<section id="history" class="history" aria-label="Commit history" data-preserve-attr="style" data-show="$sidebar === 'history'">`)
	strings.write_string(&builder, render_sidebar_search("history"))
	strings.write_string(&builder, `<ol>`)
	for commit, index in history.commits {
		if commit.working && !commit.dirty { continue }
...
<li><a id="commit-%s"%s href="%s?commit=%s&amp;sidebar=history" data-index="%d" data-on:click="if (evt.button !== 0 || evt.metaKey || evt.ctrlKey || evt.shiftKey || evt.altKey) return; evt.preventDefault(); clearTimeout($historyTimer); const current = evt.currentTarget.closest('.history').querySelector('a.selected'); if (current !== evt.currentTarget) {{ current?.classList.remove('selected'); current?.removeAttribute('aria-current'); evt.currentTarget.classList.add('selected'); evt.currentTarget.setAttribute('aria-current', 'page') }}; $selected = %d; @get('%s?commit=%s&amp;partial=1'); history.pushState(null, '', evt.currentTarget.href); document.body.dataset.pageUrl = location.pathname + location.search">
...
render_files :: proc(repository: ^Repository, initial := false) -> string {
	builder := strings.builder_make()
	defer strings.builder_destroy(&builder)
	fmt.sbprintf(
		&builder,
		`<section id="files" class="file-browser" aria-label="Markdown files" data-repository-name="%s" data-preserve-attr="style" data-show="$sidebar === 'files'" data-effect="if ($sidebar === 'files') requestAnimationFrame(() => {{ const selected = document.querySelector('.file-browser a.selected'); if (selected) {{ selected.closest('li').hidden = false; selected.scrollIntoView({{block:'nearest'}}) }} }})">
`,
	...
	for file, index in repository.files {
		...
		fmt.sbprintf(&builder, `
<li><a id="file-%d"%s href="%s" data-path="%s" data-on:click="if (evt.button !== 0 || evt.metaKey || evt.ctrlKey || evt.shiftKey || evt.altKey) return; evt.preventDefault(); const browser = evt.currentTarget.closest('.file-browser'); const current = browser.querySelector('a.selected'); if (current !== evt.currentTarget) {{ current?.classList.remove('selected'); current?.removeAttribute('aria-current'); evt.currentTarget.classList.add('selected'); evt.currentTarget.setAttribute('aria-current', 'page') }}; evt.currentTarget.focus({{preventScroll:true}}); $selected = 0; @get('%s?partial=1'); history.pushState(null, '', evt.currentTarget.href); document.body.dataset.pageUrl = location.pathname + location.search; document.title = evt.currentTarget.dataset.path + ' · ' + browser.dataset.repositoryName"><span class="file-icon" aria-hidden="true">#</span>%s</a></li>`, index, selected_class, url, html_escape(file), url, html_escape(file))
	}
	strings.write_string(&builder, "\n</ol></section>")
	return strings.clone(strings.to_string(builder))
}
...
initial_page :: proc(history: ^History, repository: ^Repository = nil, selected_commit := 0, sidebar_mode := "") -> string {
	...
	if sidebar != "id" && sidebar != "files" && sidebar != "history" && sidebar != "outline" {
		sidebar = "files"
		if repository == nil { sidebar = "history" }
	}
	if repository == nil && (sidebar == "files" || sidebar == "id") { sidebar = "history" }
...
<body data-signals="{{selected: %d, hashes: %s, sidebar: '%s', sidebarOpen: true, showChanges: false, resizing: false, idSearching: false, filesSearching: false, historySearching: false, outlineSearching: false, panel: 'sidebar', fileTimer: 0, historyTimer: 0}}"
 data-init="document.body.dataset.pageUrl = location.pathname + location.search; $idSearching = $sidebar === 'id' && new URLSearchParams(location.search).has('id-query'); try {{ $showChanges = sessionStorage.getItem('gitmd-show-changes') === 'true' }} catch {{}}"
 data-class:show-changes="$showChanges"
 data-on:popstate__window="if (location.pathname + location.search !== document.body.dataset.pageUrl) window.location.reload()"
 data-on:keydown__window="if ((($sidebar === 'id' && $idSearching) || ($sidebar === 'files' && $filesSearching) || ($sidebar === 'history' && $historySearching) || ($sidebar === 'outline' && $outlineSearching)) && evt.key === 'Escape') {{ evt.preventDefault(); const section = document.querySelector($sidebar === 'id' ? '.id-browser' : $sidebar === 'files' ? '.file-browser' : $sidebar === 'history' ? '.history' : '.outline'); const input = section.querySelector('.sidebar-search input'); input.value = ''; if ($sidebar === 'id') document.dispatchEvent(new CustomEvent('gitmd-id-clear')); else input.dispatchEvent(new Event('input')); section.querySelectorAll('li').forEach(item => item.hidden = false); if ($sidebar === 'id') $idSearching = false; else if ($sidebar === 'files') $filesSearching = false; else if ($sidebar === 'history') $historySearching = false; else $outlineSearching = false; document.querySelector('.sidebar').focus() }} else if (!evt.metaKey && !evt.ctrlKey && !evt.altKey && !evt.shiftKey && evt.key === '/') {{ evt.preventDefault(); $sidebarOpen = true; $panel = 'sidebar'; if ($sidebar === 'id') $idSearching = true; else if ($sidebar === 'files') $filesSearching = true; else if ($sidebar === 'history') $historySearching = true; else $outlineSearching = true; setTimeout(() => document.querySelector($sidebar === 'id' ? '.id-browser' : $sidebar === 'files' ? '.file-browser' : $sidebar === 'history' ? '.history' : '.outline').querySelector('.sidebar-search input').focus()) }} else if (evt.metaKey && !evt.ctrlKey && !evt.altKey && !evt.shiftKey && evt.key === 'b') {{ evt.preventDefault(); $sidebarOpen = !$sidebarOpen; $panel = $sidebarOpen ? 'sidebar' : 'main' }} else if (!evt.metaKey && !evt.ctrlKey && !evt.altKey && !evt.shiftKey && ['1','2','3','4'].includes(evt.key)) {{ evt.preventDefault(); $sidebar = ['id','files','history','outline'][Number(evt.key) - 1]; $sidebarOpen = true; $panel = 'sidebar' }} else if (!evt.metaKey && !evt.ctrlKey && !evt.altKey && !evt.shiftKey && $sidebar === 'id' && $panel === 'sidebar' && ['ArrowUp','k','ArrowDown','j','Enter'].includes(evt.key)) {{ evt.preventDefault(); document.dispatchEvent(new CustomEvent('gitmd-id-key', {{detail:evt.key}})) }} else if (!evt.metaKey && !evt.ctrlKey && !evt.altKey && !evt.shiftKey && ['ArrowUp','k','ArrowDown','j'].includes(evt.key)) {{ evt.preventDefault(); const previous = ['ArrowUp','k'].includes(evt.key); if ($panel === 'main') {{ document.querySelector('.preview-pane').scrollBy(0, previous ? -80 : 80) }} else {{ const section = document.querySelector($sidebar === 'id' ? '.id-browser' : $sidebar === 'files' ? '.file-browser' : $sidebar === 'history' ? '.history' : '.outline'); const links = section ? Array.from(section.querySelectorAll('a')).filter(link => !link.closest('li').hidden) : []; if (links.length) {{ let current = links.indexOf(section.querySelector('a.selected')); if (current < 0) current = links.indexOf(document.activeElement); if (current < 0) current = links.findIndex(link => link.getAttribute('aria-current') === 'page' || (location.hash && link.hash === location.hash)); const next = Math.max(0, Math.min(links.length - 1, current + (previous ? -1 : 1))); if (next !== current) {{ if ($sidebar === 'history') {{ const selected = section.querySelector('a.selected'); selected?.classList.remove('selected'); selected?.removeAttribute('aria-current'); links[next].classList.add('selected'); links[next].setAttribute('aria-current', 'page'); $selected = Number(links[next].dataset.index); const target = links[next]; target.focus({{preventScroll:true}}); const list = section.querySelector('ol'); const itemRect = target.getBoundingClientRect(); const listRect = list.getBoundingClientRect(); if (itemRect.top < listRect.top) list.scrollTop -= listRect.top - itemRect.top; else if (itemRect.bottom > listRect.bottom) list.scrollTop += itemRect.bottom - listRect.bottom; clearTimeout($historyTimer); $historyTimer = setTimeout(() => target.click(), 120) }} else if ($sidebar === 'files') {{ const selected = section.querySelector('a.selected'); selected?.classList.remove('selected'); selected?.removeAttribute('aria-current'); links[next].classList.add('selected'); links[next].setAttribute('aria-current', 'page'); const target = links[next]; target.focus({{preventScroll:true}}); const list = section.querySelector('ol'); const itemRect = target.getBoundingClientRect(); const listRect = list.getBoundingClientRect(); if (itemRect.top < listRect.top) list.scrollTop -= listRect.top - itemRect.top; else if (itemRect.bottom > listRect.bottom) list.scrollTop += itemRect.bottom - listRect.bottom; clearTimeout($fileTimer); $fileTimer = setTimeout(() => target.click(), 0) }} else links[next].click() }} if ($sidebar === 'outline' || next === current) links[next].focus() }} }} }}">
<main class="layout" data-class:sidebar-hidden="!$sidebarOpen" data-class:resizing="$resizing"
 data-on:keydown__window="if (!evt.metaKey && !evt.ctrlKey && !evt.altKey && !evt.shiftKey && ['ArrowLeft','h','ArrowRight','l'].includes(evt.key)) {{ evt.preventDefault(); if (['ArrowLeft','h'].includes(evt.key)) {{ $sidebarOpen = true; $panel = 'sidebar'; setTimeout(() => document.querySelector('.sidebar').focus({{preventScroll:true}})) }} else {{ $panel = 'main'; document.querySelector('.preview-pane').focus({{preventScroll:true}}) }} }}">
...
<button type="button" aria-keyshortcuts="1" data-class:selected="$sidebar === 'id'" data-on:click="$sidebar = 'id'; $panel = 'sidebar'">ID</button>
<button type="button" aria-keyshortcuts="2" data-class:selected="$sidebar === 'files'" data-on:click="$sidebar = 'files'; $panel = 'sidebar'">Files</button>
<button type="button" aria-keyshortcuts="3" data-class:selected="$sidebar === 'history'" data-on:click="$sidebar = 'history'; $panel = 'sidebar'">History</button>
<button type="button" aria-keyshortcuts="4" data-class:selected="$sidebar === 'outline'" data-on:click="$sidebar = 'outline'; $panel = 'sidebar'">Outline</button>
</nav>
...
<label><input type="checkbox" data-bind:show-changes data-on:change="try {{ sessionStorage.setItem('gitmd-show-changes', String(evt.currentTarget.checked)) }} catch {{}}"> Show changes</label>`, path, repo_name, selected_commit, hashes, sidebar, repo_name, path)
	...
	if repository != nil {
		strings.write_string(&builder, "\n")
		strings.write_string(&builder, render_repository_find())
		strings.write_string(&builder, render_files(repository, initial = true))
	}
...
.sidebar header { display:flex; flex:none; flex-direction:column; gap:4px; padding:16px; border-bottom:1px solid var(--border); }
.sidebar header strong { font-size:18px; }.sidebar header span { color:var(--muted); overflow-wrap:anywhere; line-height:1.2; height:2.4em; display:-webkit-box; -webkit-box-orient:vertical; -webkit-line-clamp:2; overflow:hidden; }
.sidebar-tabs { display:grid; flex:none; grid-template-columns:repeat(4,minmax(0,1fr)); padding:6px; gap:2px; border-bottom:1px solid var(--border); background:var(--subtle); }
.sidebar-tabs button { padding:7px 3px; min-width:0; border:1px solid transparent; border-radius:6px; background:transparent; color:var(--muted); cursor:pointer; font-weight:600; }.sidebar-tabs button:hover { color:var(--fg); }.sidebar-tabs button.selected { color:var(--fg); border-color:var(--border); background:var(--bg); }
...
.id-browser { display:flex; flex:1; min-height:0; flex-direction:column; overflow:hidden; }
.id-browser>ol { flex:1; min-height:0; overflow:auto; list-style:none; margin:0; padding:8px; }
.id-browser a { display:block; padding:8px; border-radius:6px; color:inherit; text-decoration:none; overflow-wrap:anywhere; }
.id-browser ol ol { list-style:none; margin:0 0 0 12px; padding:0 0 0 5px; border-left:1px solid var(--border); }
.id-tree-row { display:flex; align-items:center; min-width:0; }
.id-browser .id-tree-toggle { flex:none; width:20px; padding:5px 0; border:0; background:transparent; color:var(--muted); cursor:pointer; }
.id-browser .id-tree-toggle[hidden] { display:block; visibility:hidden; }
.id-browser .id-tree-toggle:disabled { cursor:default; }
.id-browser .id-tree-label { display:block; flex:1; min-width:0; padding:5px 4px; border:0; border-radius:5px; background:transparent; color:inherit; text-align:left; font:inherit; cursor:pointer; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; }
.id-tree-label strong { font-family:ui-monospace,SFMono-Regular,Consolas,monospace; font-size:12px; }
.id-tree-label strong+span { margin-left:7px; color:var(--muted); }
.id-browser .id-tree-label.selected { background:var(--selected); }
.id-browser .id-tree-label:hover { background:color-mix(in srgb,var(--selected) 55%,transparent); }
.id-browser mark { background:var(--selected); color:var(--accent); font-weight:600; }
.id-browser button:focus-visible,.id-browser a:focus-visible { outline:2px solid var(--accent); outline-offset:-2px; }
.id-browser a:hover { background:color-mix(in srgb,var(--selected) 55%,transparent); }
.id-browser small { display:block; color:var(--muted); font-size:12px; margin-top:4px; }
.id-status { flex:none; padding:8px 16px; margin:0; color:var(--muted); font-size:12px; }
