...
snapshot_routes_are_fragments :: proc(t: ^testing.T) {
	...
	testing.expect(t, strings.contains(page, `const links = section ? Array.from(section.querySelectorAll('a')) : []`))
	testing.expect(t, strings.contains(page, `links[next].scrollIntoView({block:'nearest'})`))
	testing.expect(t, strings.contains(page, `let current = $sidebar === 'history' ? $selected`))
	testing.expect(t, strings.contains(page, `$fileTimer = setTimeout(() => links[next].click(), 0)`))
	testing.expect(t, strings.contains(page, `$historyTimer = setTimeout(() => links[$selected].click(), 0)`))
