...
snapshot_routes_are_fragments :: proc(t: ^testing.T) {
	...
	testing.expect(t, strings.contains(page, `['1','2','3','4'].includes(evt.key)`))
	testing.expect(t, strings.contains(page, `$sidebar = ['id','files','history','outline'][Number(evt.key) - 1]`))
...
repository_page_has_file_and_history_modes :: proc(t: ^testing.T) {
	...
	testing.expect(t, strings.contains(page, ">ID</button>"))
	testing.expect(t, strings.contains(page, `id="id-search-input"`))
	testing.expect(t, strings.contains(page, `aria-keyshortcuts="4"`))
	...
	list_position := files_position + strings.index(page[files_position:], `<ol>`)
